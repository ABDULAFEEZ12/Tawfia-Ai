...
hf\_token = os.getenv("HUGGINGFACE\_API\_TOKEN")
...
json.dump(question\_cache, f, indent=2, ensure\_ascii=False)

# --- Load JSON datasets ---

def load\_json\_data(file\_name, data\_variable\_name):
data = {}
file\_path = os.path.join(os.path.dirname(**file**), 'DATA', file\_name)
print(f"Attempting to load {data\_variable\_name} data from: {file\_path}")
try:
with open(file\_path, 'r', encoding='utf-8') as f:
data = json.load(f)
print(f"✅ Successfully loaded {data\_variable\_name} data")
except FileNotFoundError:
print(f"❌ ERROR: {data\_variable\_name} data file not found at {file\_path}")
except json.JSONDecodeError as e:
print(f"❌ JSON Decode Error in {file\_path}: {e}")
except Exception as e:
print(f"❌ Unexpected error while loading {file\_name}: {e}")
return data

# Load local data

hadith\_data = load\_json\_data('sahih\_bukhari\_coded.json', 'Hadith')
basic\_knowledge\_data = load\_json\_data('basic\_islamic\_knowledge.json', 'Basic Islamic Knowledge')
friendly\_responses\_data = load\_json\_data('friendly\_responses.json', 'Friendly Responses')
...
def get\_surah\_list():
try:
response = requests.get('[https://api.quran.gading.dev/surah](https://api.quran.gading.dev/surah)')
response.raise\_for\_status()
surahs = response.json()\['data']
names = \[s\['name']\['transliteration']\['en'] for s in surahs]
return jsonify({'surah\_list': names})
except requests.RequestException as e:
print(f"Surah List API Error: {e}")
return jsonify({'surah\_list': \[]})
