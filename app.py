...
@app.route('/ask-mobile', methods=['POST'])
def ask_mobile():
    data = request.get_json()
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are Tawfiq AI — a wise, kind, and emotionally intelligent Muslim assistant. "
                "Respond clearly, use halal-friendly language, and always maintain adab."
            )
        },
    ...
    try:
        ...
        print("❌ Mobile ask error:", e)
        return jsonify({"reply": "Tawfiq AI is facing issues. Try again later."})
