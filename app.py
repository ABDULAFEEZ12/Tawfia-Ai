...
def ask():
    ...
    if friendly_responses_data:
        ...
        if close_friendly_matches:
            match_key = close_friendly_matches[0]
            print(f"✨ Found close match in friendly_responses for: {question} -> {match_key}")
            return jsonify({'answer': friendly_responses_data[match_key]})
    ...
    try:
        ...
        islamic_prompt = (
            ...
            " Do not provide opinions or info from other sources. Focus on factual Islamic knowledge."
...
def quran_search():
    ...
    print(f"Received Quran search query: '{query}'")  # Debug
    ...
    try:
        ...
        # Map transliterated names to surah number
        ...
        if close_matches:
            ...
            print(f"Returning {len(structured_verses)} verses for Surah {surah_title}")
...
def hadith_search():
    data = request.get_json()
    query = data.get('query', '').strip().lower()
    print(f"Received Hadith search query: '{query}'")  # Debug
    ...
    try:
        ...
        if structured_matches:
            print(f"Found {len(structured_matches)} Hadith matches")
            return jsonify({'results': structured_matches})
        else:
            print("No Hadith matches found")
